...
void index_generator::generate(integral_curves& integral_curves, const bool use_64_bit)
{
  for (auto& curve : integral_curves)
  {
    if (use_64_bit)
    {
      ...
      auto cast_indices = std::get<std::vector<std::uint64_t>>(curve.indices);
      ...
      tbb::parallel_for(std::size_t(0), curve.vertices.size() - 1, std::size_t(1), [&] (const std::size_t vertex_index)
      {
        ...
        {
          cast_indices.at(2 * vertex_index    ) = vertex_index;
          cast_indices.at(2 * vertex_index + 1) = vertex_index + 1;
        }
      });
    ...
    else
    {
      ...
      auto cast_indices = std::get<std::vector<std::uint32_t>>(curve.indices);
